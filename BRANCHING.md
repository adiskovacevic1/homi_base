...
**4. Catch up before you merge**, so the conflict is yours to fix and not dev's:

```bash
git fetch origin && git merge origin/dev      # resolve here, on your branch
```
...
- **Do not rewrite shared history.** No rebasing `dev`, `main` or `stable` after pushing.
