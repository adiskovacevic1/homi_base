...
search; when it is on (the default, `BRAIN_WEB_SEARCH=1`) the kit's `web_search` steps aside for Claude so tool names
stay unique. DeepSeek and OpenAI have no browsing of their own here and use the kit's tools.
